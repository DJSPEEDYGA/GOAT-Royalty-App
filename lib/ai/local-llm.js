...
// Build default model search paths from environment.
// GOAT_MODEL_PATHS can be a comma-separated list of directories.
function getDefaultModelPaths() {
    const paths = [];
    if (process.env.GOAT_MODEL_PATHS) {
        process.env.GOAT_MODEL_PATHS.split(',').forEach(p => {
            const trimmed = p.trim();
            if (trimmed) paths.push(trimmed);
        });
    }
    // Always include the legacy data-path/models directory
    paths.push(path.join(DEFAULT_DATA_PATH, MODELS_DIR));
    return paths;
}
...
class LocalLLMEngine extends EventEmitter {
    constructor(options = {}) {
        ...
        this.modelsPaths = options.modelsPaths || getDefaultModelPaths();
    ...
    _initialize() {
        ...
        if (!fs.existsSync(this.contextPath)) {
            ...
            console.log(`[LocalLLM] Created directory: ${this.contextPath}`);
        }

        // Ensure all model directories exist (create missing ones so users know where to copy)
        this.modelsPaths.forEach(dir => {
            if (!fs.existsSync(dir)) {
                fs.mkdirSync(dir, { recursive: true });
                console.log(`[LocalLLM] Created model directory: ${dir}`);
            }
        });
        ...
        console.log(`[LocalLLM] Model search paths: ${this.modelsPaths.join(', ')}`);
            ...
            try {
                ...
                files.forEach(file => {
                    if (file.endsWith('.gguf') || file.endsWith('.bin')) {
                        ...
                        const modelInfo = this._parseModelFile(file, filePath);
                        // Deduplicate by full path so identical filenames in different folders are both visible
                        this.models.set(modelInfo.path, modelInfo);
                    }
                });
            } catch (error) {
                console.warn(`[LocalLLM] Error scanning models in ${modelsPath}:`, error.message);
            }
        }
    }

    _parseModelFile(filename, filePath) {
    ...
    getModelInfo(modelIdOrPath) {
        // Support lookup by full path or by basename id
        if (this.models.has(modelIdOrPath)) return this.models.get(modelIdOrPath);
        return Array.from(this.models.values()).find(m => m.id === modelIdOrPath);
    }

    setDataPath(newPath) {
        // Change data path (e.g., to external drive). Keeps any GOAT_MODEL_PATHS env paths.
        this.dataPath = newPath;
        this.modelsPaths = getDefaultModelPaths();
        this.contextPath = path.join(newPath, CONTEXT_DIR);
        this._initialize();
        return this.dataPath;
    }

    setModelsPaths(paths) {
        if (!Array.isArray(paths)) throw new Error('modelsPaths must be an array');
        this.modelsPaths = paths;
        this._initialize();
        return this.modelsPaths;
    }
    ...
    getDownloadInstructions(modelName) {
        ...
        if (info) {
            const primaryModelPath = this.modelsPaths[0] || path.join(this.dataPath, MODELS_DIR);
            return {
                name: modelName,
                ...info,
                destination: primaryModelPath,
                instructions: `1. Download ${info.file} from ${info.url}\n2. Copy to: ${primaryModelPath}\n3. Restart the app to detect the model`
            };
        }
