...
// API Keys Configuration (loaded from environment, not committed)
const API_CONFIG = {
    superninja: process.env.SUPER_NINJA_API_KEY || '',
    hostinger: process.env.HOSTINGER_API_KEY || '',
    supabase: process.env.SUPABASE_KEY || '',
    googleAI: process.env.GOOGLE_AI_STUDIO_KEY || '',
    fashionApp: process.env.FASHION_APP_API_KEY || ''
};

// Server Configuration (loaded from environment, not committed)
const SERVER_CONFIG = {
    ip: process.env.SERVER_IP || 'localhost',
    os: process.env.SERVER_OS || 'Ubuntu',
    resources: {
        cpu: parseInt(process.env.SERVER_CPU, 10) || 2,
        ram: process.env.SERVER_RAM || '8GB',
        disk: process.env.SERVER_DISK || '386GB'
    },
    location: process.env.SERVER_LOCATION || 'Local'
};
