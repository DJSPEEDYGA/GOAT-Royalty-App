#!/bin/bash
# Launch Agent 007 chat server on port 3334.
set -euo pipefail
...
if curl -s --max-time 2 http://127.0.0.1:$PORT/api/stats > /dev/null 2>&1; then
  echo "[OK] Agent 007 chat server is already running on http://127.0.0.1:$PORT"
  exit 0
fi
...
echo "Starting Agent 007 chat server on http://127.0.0.1:$PORT..."
